...
router.post('/register', async (req, res) => {
  try {
    ...
    try {
      ...
      // Handle session ID if provided (for transferring anonymous emails)
      const transferResults = { transferred: 0 };
      
      if (sessionId) {
        // Set new expiry date (2 months from now) for transferred emails
        const newExpiryDate = new Date();
        newExpiryDate.setMonth(newExpiryDate.getMonth() + 2);
        
        // Update all emails associated with this session ID to belong to the new user
        const [updateResult] = await connection.query(
          `UPDATE temp_emails 
           SET user_id = ?, 
               session_id = NULL, 
               created_by_anonymous = FALSE, 
               expires_at = ? 
           WHERE session_id = ? AND created_by_anonymous = TRUE`,
          [id, newExpiryDate, sessionId]
        );
        
        transferResults.transferred = updateResult.affectedRows || 0;
      }
