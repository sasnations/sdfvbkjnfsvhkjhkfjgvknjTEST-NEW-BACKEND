...
router.get('/public', async (req, res) => {
  try {
    // Check cache first
    const cachedDomains = cacheManager.get('domains', 'public');
    if (cachedDomains) {
    ...
    // Cache the result
    cacheManager.set('domains', 'public', domains);
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    // Check cache first
    const cachedDomains = cacheManager.get('domains', 'all');
    if (cachedDomains) {
      return res.json(cachedDomains);
    }
    ...
    // Cache the result
    cacheManager.set('domains', 'all', domains);
...
router.post('/add', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    cacheManager.invalidateType('domains');
