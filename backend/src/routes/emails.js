...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    // Check cache first
    const cachedEmail = cacheManager.get('tempEmails', req.params.id, { userId: req.user.id });
    if (cachedEmail) {
    ...
    // Cache the result
    cacheManager.set('tempEmails', req.params.id, emails[0], { userId: req.user.id });
...
router.get('/:id/received', authenticateToken, async (req, res) => {
  try {
    ...
    const cacheKey = `${req.params.id}_${page}_${limit}`;
    
    // Check cache first
    const cachedEmails = cacheManager.get('receivedEmails', cacheKey, { userId: req.user.id });
    if (cachedEmails) {
    ...
    // Cache the result
    cacheManager.set('receivedEmails', cacheKey, response, { userId: req.user.id });
...
router.post('/create', authenticateToken, rateLimitMiddleware, checkCaptchaRequired, verifyCaptcha, async (req, res) => {
  try {
    ...
    try {
      ...
      // Invalidate cache for user's emails
      cacheManager.invalidateType('tempEmails');
...
router.delete('/delete/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Invalidate cache for this email and user's emails
    cacheManager.delete('tempEmails', req.params.id, { userId: req.user.id });
    cacheManager.invalidateType('receivedEmails');
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Generate cache key based on pagination and search params
    const cacheKey = `user_${req.user.id}_${page}_${limit}_${search}`;
    
    // Check cache first
    const cachedResult = cacheManager.get('tempEmails', 'list', { key: cacheKey });
    if (cachedResult) {
    ...
    // Cache the result
    cacheManager.set('tempEmails', 'list', response, { key: cacheKey });
...
router.delete('/:tempEmailId/received/:emailId', authenticateToken, async (req, res) => {
  try {
    ...
    // Invalidate cache for received emails
    cacheManager.invalidateType('receivedEmails');
...
router.get('/public/:email', async (req, res) => {
  try {
    ...
    // Check cache first
    const cachedEmails = cacheManager.get('receivedEmails', 'public', { email: req.params.email });
    if (cachedEmails) {
      return res.json(cachedEmails);
    }
    ...
    // Cache the result
    cacheManager.set('receivedEmails', 'public', emails, { email: req.params.email });
...
router.post('/public/create', rateLimitMiddleware, checkCaptchaRequired, verifyCaptcha, async (req, res) => {
  try {
    ...
    try {
      ...
      // Invalidate public emails cache
      cacheManager.invalidateType('tempEmails');
...
router.get('/admin/all', async (req, res) => {
  try {
    ...
    // Check cache for admin emails
    const cacheKey = `admin_all_${page}_${limit}`;
    const cachedEmails = cacheManager.get('receivedEmails', 'admin', { key: cacheKey });
    if (cachedEmails) {
      return res.json(cachedEmails);
    }
    ...
    // Cache the result
    cacheManager.set('receivedEmails', 'admin', response, { key: cacheKey });
...
router.get('/admin/users', async (req, res) => {
  try {
    ...
    const cacheKey = `admin_users_${search || ''}_${sortBy}_${sortOrder}_${page}_${limit}_${emailCountMin || ''}_${emailCountMax || ''}_${dateStart || ''}_${dateEnd || ''}_${isActive || ''}_${hasCustomDomain || ''}`;
    
    // Check cache first
    const cachedResult = cacheManager.get('users', 'admin', { key: cacheKey });
    if (cachedResult) {
      return res.json(cachedResult);
    }
    ...
    // Cache the result
    cacheManager.set('users', 'admin', response, { key: cacheKey }, 5 * 60 * 1000); // 5 minute TTL for admin queries
