...
router.get('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const cachedMessages = cacheManager.get('messages', 'user', { key: cacheKey });
    if (cachedMessages) {
    ...
    // Cache the result
    cacheManager.set('messages', 'user', messages, { key: cacheKey });
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    cacheManager.invalidateType('messages');
...
router.post('/:id/dismiss', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Invalidate user's messages cache
    const cacheKey = `messages_${userId}`;
    cacheManager.delete('messages', 'user', { key: cacheKey });
...
router.get('/admin/all', async (req, res) => {
  ...
  try {
    // Check cache first
    const cachedMessages = cacheManager.get('messages', 'admin');
    if (cachedMessages) {
      return res.json(cachedMessages);
    }
    ...
    // Cache the result
    cacheManager.set('messages', 'admin', messages);
