...
// Initialize cache from database for fast startup
async function initializeCache() {
  console.log('Initializing email cache from database...');
  try {
    // Fetch active temporary emails
    const [activeEmails] = await pool.query(`
      SELECT * FROM temp_emails 
      WHERE expires_at > NOW() 
      LIMIT 5000
    `);

    console.log(`Loading ${activeEmails.length} active temporary emails into cache...`);
    
    // Add emails to cache
    for (const email of activeEmails) {
      emailCacheManager.addTempEmail(email);
      
      // Also fetch recent received emails for each temp email
      ...
      // Add received emails to cache
      receivedEmails.forEach(receivedEmail => {
        emailCacheManager.addReceivedEmail(email.id, receivedEmail);
      });
    }
    
    console.log('Email cache initialization complete');
  } catch (error) {
...
initializeDatabase().then(async () => {
  // Initialize cache after database connection is established
