...
router.get('/posts', publicRateLimit, async (req, res) => {
  try {
    // Check cache first
    const cachedPosts = cacheManager.get('blogPosts', 'all');
    if (cachedPosts) {
    ...
    // Cache the results
    cacheManager.set('blogPosts', 'all', postsWithMeta);
...
router.get('/posts/:slug', publicRateLimit, async (req, res) => {
  try {
    // Check cache first
    const cachedPost = cacheManager.get('blogPosts', req.params.slug);
    if (cachedPost) {
    ...
    // Cache the result
    cacheManager.set('blogPosts', req.params.slug, post);
...
router.post('/posts', async (req, res) => {
  ...
  try {
    ...
    cacheManager.invalidateType('blogPosts');
    cacheManager.invalidateType('blogCategories');
...
router.put('/posts/:id', async (req, res) => {
  ...
  try {
    ...
    // Invalidate blog posts cache
    cacheManager.invalidateType('blogPosts');
    cacheManager.delete('blogPosts', slug);
    cacheManager.delete('blogPosts', req.params.id);
...
router.get('/categories', publicRateLimit, async (req, res) => {
  try {
    // Check cache first
    const cachedCategories = cacheManager.get('blogCategories', 'all');
    if (cachedCategories) {
    ...
    // Cache the results
    cacheManager.set('blogCategories', 'all', categoryList);
...
router.get('/posts/featured', publicRateLimit, async (req, res) => {
  try {
    // Check cache first
    const cachedPosts = cacheManager.get('blogPosts', 'featured');
    if (cachedPosts) {
      return res.json(cachedPosts);
    }
    ...
    // Cache the results
    cacheManager.set('blogPosts', 'featured', postsWithMeta);
...
router.get('/posts/trending', publicRateLimit, async (req, res) => {
  try {
    // Check cache first
    const cachedPosts = cacheManager.get('blogPosts', 'trending');
    if (cachedPosts) {
      return res.json(cachedPosts);
    }
    ...
    // Cache the results
    cacheManager.set('blogPosts', 'trending', postsWithMeta);
