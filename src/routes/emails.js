...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    // Check cache first
    const cachedEmail = emailCacheManager.getTempEmailById(req.params.id);
    if (cachedEmail && cachedEmail.user_id === req.user.id) {
    ...
    emailCacheManager.addTempEmail(emails[0]);
...
router.get('/:id/received', authenticateToken, async (req, res) => {
  try {
    ...
    // Check cache first
    const cachedEmails = emailCacheManager.getReceivedEmails(req.params.id, page, limit);
    if (cachedEmails) {
    ...
    // Cache each email and store the list
    emails.forEach(email => {
      emailCacheManager.addReceivedEmail(req.params.id, email);
    });
...
router.post('/create', authenticateToken, rateLimitMiddleware, checkCaptchaRequired, verifyCaptcha, async (req, res) => {
  try {
    ...
    try {
      ...
      // Add email to cache immediately after creation
      const newEmail = createdEmail[0];
      emailCacheManager.addTempEmail(newEmail);
...
router.delete('/:tempEmailId/received/:emailId', authenticateToken, async (req, res) => {
  try {
    ...
    emailCacheManager.removeReceivedEmail(req.params.tempEmailId, req.params.emailId);
...
router.get('/public/:email', async (req, res) => {
  try {
    ...
    // If no emails in memory, fall back to database (using the existing cache as well)
    console.log(`No emails in memory for ${requestedEmail}, checking database...`);
    
    // Check cache first by email address (existing code)
    const tempEmail = emailCacheManager.getTempEmailByAddress(requestedEmail);
    if (tempEmail) {
      const cachedEmails = emailCacheManager.getReceivedEmails(tempEmail.id);
      if (cachedEmails) {
        return res.json(cachedEmails.data);
      }
    }
    
    // If not in cache or memory, fetch from database
    const [emails] = await pool.query(`
      SELECT re.*, te.email as temp_email
      FROM received_emails re
      JOIN temp_emails te ON re.temp_email_id = te.id
    ...
    // Cache these results if we found the temp email (keeping existing code)
    if (emails.length > 0 && emails[0].temp_email) {
      const relatedTempEmail = await pool.query(
        'SELECT * FROM temp_emails WHERE email = ?',
        [requestedEmail]
      );
      
      if (relatedTempEmail[0] && relatedTempEmail[0].length > 0) {
        emailCacheManager.addTempEmail(relatedTempEmail[0][0]);
        emails.forEach(email => {
          emailCacheManager.addReceivedEmail(relatedTempEmail[0][0].id, email);
        });
      }
    }
