...
router.get('/stats', async (req, res) => {
  ...
  // Check cache first
  const cachedStats = cacheManager.get('stats', 'overview');
  if (cachedStats) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('stats', 'overview', stats);
...
router.post('/cleanup', async (req, res) => {
  ...
  try {
    ...
    // Invalidate relevant caches after cleanup
...
router.get('/recent-users', async (req, res) => {
  ...
  // Check cache first
  const cachedUsers = cacheManager.get('users', 'recent');
  if (cachedUsers) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('users', 'recent', users);
...
router.get('/user-activity', async (req, res) => {
  ...
  // Check cache first
  const cachedActivity = cacheManager.get('stats', 'userActivity');
  if (cachedActivity) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('stats', 'userActivity', activity);
...
router.get('/top-users', async (req, res) => {
  ...
  // Check cache first
  const cachedUsers = cacheManager.get('users', 'top');
  if (cachedUsers) {
    return res.json(cachedUsers);
  }
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('users', 'top', topUsers);
...
router.get('/recent-ips', async (req, res) => {
  ...
  const cacheKey = `recent_ips_${limit}`;
  const cachedIps = cacheManager.get('ips', 'recent', { limit });
  if (cachedIps) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('ips', 'recent', recentIps, { limit });
...
router.get('/lookup-ip', async (req, res) => {
  ...
  // Check cache first
  const cachedIpInfo = cacheManager.get('ips', ip);
  if (cachedIpInfo) {
    return res.json(cachedIpInfo);
  }
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('ips', ip, result);
...
router.get('/lookup-request', async (req, res) => {
  ...
  // Check cache first
  const cachedRequest = cacheManager.get('requests', requestId);
  if (cachedRequest) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('requests', requestId, result);
...
router.get('/request-stats', async (req, res) => {
  ...
  // Check cache first
  const cachedStats = cacheManager.get('stats', 'requests');
  if (cachedStats) {
    return res.json(cachedStats);
  }
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('stats', 'requests', result);
...
router.get('/ip-behaviors', async (req, res) => {
  ...
  // Check cache first
  const cachedBehaviors = cacheManager.get('ips', 'behaviors');
  if (cachedBehaviors) {
    return res.json(cachedBehaviors);
  }
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('ips', 'behaviors', behaviors);
...
router.get('/blocked-ips', async (req, res) => {
  ...
  // Check cache first
  const cachedIps = cacheManager.get('ips', 'blocked');
  if (cachedIps) {
    return res.json(cachedIps);
  }
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('ips', 'blocked', blockedIps);
...
router.post('/block-ip', async (req, res) => {
  ...
  try {
    ...
    cacheManager.delete('ips', 'blocked');
...
router.get('/lookup-email-ip', async (req, res) => {
  ...
  // Check cache first
  const cachedResult = cacheManager.get('emailIp', email);
  if (cachedResult) {
  ...
  try {
    ...
    // Cache the result
    cacheManager.set('emailIp', email, result);

    console.log('Found IP info:', result);
    res.json(result);
