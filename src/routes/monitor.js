...
// Add caching statistics to monitoring
router.get('/cache-stats', async (req, res) => {
  if (!checkAdminPassphrase(req)) {
    return res.status(403).json({ error: 'Unauthorized' });
  }

  const stats = emailCacheManager.getStats();
  res.json(stats);
});

// Add memory store statistics to monitoring
router.get('/memory-store-stats', async (req, res) => {
  if (!checkAdminPassphrase(req)) {
    return res.status(403).json({ error: 'Unauthorized' });
  }

  const stats = emailMemoryStore.getStats();
  res.json(stats);
});
